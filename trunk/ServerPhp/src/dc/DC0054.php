 ...
 * Generated content.
 * DC0054 Controller: Project component types def.
 */

require_once(PATH_N21LIB."/Http/Controller.php");

class DC0054 extends Controller {
...
public function fetchRecord() {
    ...
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0054.fetchRecord().");}
...
public function doInsert() {
    ...
    $_seq = $this->getDbConn()->execute("select seq_prjcmptyp_id.nextval seq_val from dual")->fetchRow();
...
public function doDelete() {
    $this->record["ID"] = $this->getRequestParam("ID");
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0054.doDelete().");}
    $sql = "delete from PROJECT_CMP_TYPE where 
           ID= :ID
    ";
    $stmt = $this->getDbConn()->prepare($sql);
    $this->getDbConn()->Execute($stmt, $this->record);
...
public function initNewRecord() {
    ...
    $this->setFieldInitialValues($this->record, "DC0054");
    print "{success:true, data:".json_encode($this->record)."}";
