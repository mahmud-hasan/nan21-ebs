 ...
 * Generated content.
 * DC0087 Controller: Stock location types
 */

require_once(PATH_N21LIB."/Http/Controller.php");

class DC0087 extends Controller {
...
public function doQuery() {
    ...
    $sql = "select 
                ...
                ,t.MODIFIEDBY
                ,t.MODIFIEDON
            from MM_STOCK_LOC_TYPE t $where $orderByClause ";
...
public function fetchRecord() {
    ...
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0087.fetchRecord().");}
...
public function doInsert() {
    ...
    $sql = "insert into MM_STOCK_LOC_TYPE(
                 CODE
                ,ID
            ) values ( 
                 :CODE
    ...
    $_seq = $this->getDbConn()->execute("select SEQ_STOCKLOCTYP_ID.nextval seq_val from dual")->fetchRow();
...
public function doUpdate() {
    ...
    $this->record["_p_record_status"] = $this->getRequestParam("_p_record_status");
    $this->record["_p_store_recId"] = $this->getRequestParam("_p_store_recId");
    $sql = "update MM_STOCK_LOC_TYPE set 
                 CODE=:CODE
                ,CREATEDBY=:CREATEDBY
                ,CREATEDON=:CREATEDON
                ,ID=:ID
...
public function doDelete() {
    $this->record["ID"] = $this->getRequestParam("ID");
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0087.doDelete().");}
    $sql = "delete from MM_STOCK_LOC_TYPE where 
           ID= :ID
    ";
    $stmt = $this->getDbConn()->prepare($sql);
    $this->getDbConn()->Execute($stmt, $this->record);
...
public function initNewRecord() {
    ...
    $this->setFieldInitialValues($this->record, "DC0087");
    print "{success:true, data:".json_encode($this->record)."}";
