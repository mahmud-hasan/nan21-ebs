 ...
 * Generated content.
 * DC0112 Controller: Parcel event types
 */

require_once(PATH_N21LIB."/Http/Controller.php");

class DC0112 extends Controller {
...
private function preQuery(&$params, &$where) {
    ...
    if (!empty($_REQUEST["QRY_EVNTGRP_CODE"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "t.EVNTGRP_CODE like :EVNTGRP_CODE";
      $params["EVNTGRP_CODE"] = $_REQUEST["QRY_EVNTGRP_CODE"];
    }
...
public function doQuery() {
    ...
    $sql = "select 
                ...
                ,t.MODIFIEDON
                ,t.NAME
            from TR_PARCEL_EVENT_TYPE t $where $orderByClause ";
...
public function fetchRecord() {
    ...
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0112.fetchRecord().");}
...
public function doInsert() {
    ...
    $sql = "insert into TR_PARCEL_EVENT_TYPE(
                 ACTIVE
                ,CODE
                ,EVNTGRP_CODE
                ,ID
    ...
    $_seq = $this->getDbConn()->execute("select SEQ_PARCELEVNTTYP_ID.nextval seq_val from dual")->fetchRow();
...
public function doUpdate() {
    ...
    $this->record["_p_record_status"] = $this->getRequestParam("_p_record_status");
    $this->record["_p_store_recId"] = $this->getRequestParam("_p_store_recId");
    $sql = "update TR_PARCEL_EVENT_TYPE set 
                 ACTIVE=:ACTIVE
                ,CREATEDBY=:CREATEDBY
                ,CREATEDON=:CREATEDON
...
public function doDelete() {
    $this->record["ID"] = $this->getRequestParam("ID");
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0112.doDelete().");}
    $sql = "delete from TR_PARCEL_EVENT_TYPE where 
           ID= :ID
    ";
    $stmt = $this->getDbConn()->prepare($sql);
    $this->getDbConn()->Execute($stmt, $this->record);
...
public function initNewRecord() {
    ...
    $this->setFieldInitialValues($this->record, "DC0112");
    print "{success:true, data:".json_encode($this->record)."}";
...
private function _initFields() {
  ...
  ,"EVNTGRP_CODE" => array(parent::FLDPROP_DATA_TYPE => "STRING")
