 ...
 * Generated content.
 * DC0106 Controller: Documents serial no. range
 */

require_once(PATH_N21LIB."/Http/Controller.php");

class DC0106 extends Controller {
...
private function preQuery(&$params, &$where) {
    if (!empty($_REQUEST["QRY_DOCSER_ID"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "t.DOCSER_ID like :DOCSER_ID";
      $params["DOCSER_ID"] = $_REQUEST["QRY_DOCSER_ID"];
    }
...
public function doQuery() {
    ...
    $sql = "select 
                ...
                ,t.MAXVAL
                ,t.MINVAL
            from DOCUMENT_SERIAL_RANGE t $where $orderByClause ";
...
public function fetchRecord() {
    ...
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0106.fetchRecord().");}
...
public function doInsert() {
    ...
    $sql = "insert into DOCUMENT_SERIAL_RANGE(
                 DOCSER_ID
                ,ID
                ,MAXVAL
                ,MINVAL
    ...
    $_seq = $this->getDbConn()->execute("select SEQ_DOCSERRNG_ID.nextval seq_val from dual")->fetchRow();
...
public function doUpdate() {
    ...
    $this->record["_p_record_status"] = $this->getRequestParam("_p_record_status");
    $this->record["_p_store_recId"] = $this->getRequestParam("_p_store_recId");
    $sql = "update DOCUMENT_SERIAL_RANGE set 
                 DOCSER_ID=:DOCSER_ID
                ,ID=:ID
...
public function doDelete() {
    $this->record["ID"] = $this->getRequestParam("ID");
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0106.doDelete().");}
    $sql = "delete from DOCUMENT_SERIAL_RANGE where 
           ID= :ID
    ";
    $stmt = $this->getDbConn()->prepare($sql);
    $this->getDbConn()->Execute($stmt, $this->record);
...
public function initNewRecord() {
    ...
    $this->setFieldInitialValues($this->record, "DC0106");
    print "{success:true, data:".json_encode($this->record)."}";
...
private function _initFields() {
  $this->fields = array(
  "DOCSER_ID" => array(parent::FLDPROP_DATA_TYPE => "NUMBER")
  ,"ID" => array(parent::FLDPROP_DATA_TYPE => "NUMBER")
  ,"IS_CLOSED" => array(parent::FLDPROP_DATA_TYPE => "BOOLEAN")
  ,"IS_INSTALLED" => array(parent::FLDPROP_DATA_TYPE => "BOOLEAN")
  ,"MAXVAL" => array(parent::FLDPROP_DATA_TYPE => "NUMBER")
  ,"MINVAL" => array(parent::FLDPROP_DATA_TYPE => "NUMBER")
);
}
