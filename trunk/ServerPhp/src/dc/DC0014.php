 ...
 * Generated content.
 * DC0014 Controller: Business partner master data
 */
...
public function fetchRecord() {
    ...
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0014.fetchRecord().");}
...
public function doInsert() {
    ...
    $_seq = $this->getDbConn()->execute("select seq_bpartner_id.nextval seq_val from dual")->fetchRow();
...
public function doUpdate() {
    $this->populateRecordFromRequest();
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0014.doUpdate().");}
...
public function doDelete() {
    $this->record["ID"] = $this->getRequestParam("ID");
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0014.doDelete().");}
...
public function initNewRecord() {
    ...
    $this->setFieldInitialValues($this->record, "DC0014");
    print "{success:true, data:".json_encode($this->record)."}";
...
private function _initFields() {
  ...
  ,"BPARTNER_TYPE" => array(parent::FLDPROP_DATA_TYPE => "STRING")
  ,"CODE" => array(parent::FLDPROP_DATA_TYPE => "STRING",parent::FLDPROP_CASE_RESTRICTION => "Upper")
  ,"COMPANY_REG_NR" => array(parent::FLDPROP_DATA_TYPE => "STRING")
  ,"COPIED_FROM_BPARTNER_ID" => array(parent::FLDPROP_DATA_TYPE => "NUMBER")
  ...
  ,"TAX_NUMBER" => array(parent::FLDPROP_DATA_TYPE => "STRING")
  ,"TAX_NUMBER_TYPE" => array(parent::FLDPROP_DATA_TYPE => "STRING")
);
}
