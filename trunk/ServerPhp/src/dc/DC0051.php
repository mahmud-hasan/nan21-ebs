 ...
 * Generated content.
 * DC0051 Controller: Project issue status def.
 */

require_once(PATH_N21LIB."/Http/Controller.php");

class DC0051 extends Controller {
...
public function fetchRecord() {
    ...
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0051.fetchRecord().");}
...
public function doInsert() {
    ...
    $_seq = $this->getDbConn()->execute("select seq_prjisssts_id.nextval seq_val from dual")->fetchRow();
...
public function doDelete() {
    $this->record["ID"] = $this->getRequestParam("ID");
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0051.doDelete().");}
    $sql = "delete from PROJECT_ISSUE_STATUS where 
           ID= :ID
    ";
    $stmt = $this->getDbConn()->prepare($sql);
    $this->getDbConn()->Execute($stmt, $this->record);
...
public function initNewRecord() {
    ...
    $this->setFieldInitialValues($this->record, "DC0051");
    print "{success:true, data:".json_encode($this->record)."}";
