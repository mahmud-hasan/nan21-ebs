 ...
 * Generated content.
 * DC0010 Controller: UoM types
 */

require_once(PATH_N21LIB."/Http/Controller.php");

class DC0010 extends Controller {
...
public function fetchRecord() {
    ...
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0010.fetchRecord().");}
...
public function doInsert() {
    ...
    $_seq = $this->getDbConn()->execute("select seq_uomtype_id.nextval seq_val from dual")->fetchRow();
...
public function doDelete() {
    $this->record["ID"] = $this->getRequestParam("ID");
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0010.doDelete().");}
    $sql = "delete from UOM_TYPE where 
           ID= :ID
    ";
    $stmt = $this->getDbConn()->prepare($sql);
    $this->getDbConn()->Execute($stmt, $this->record);
...
public function initNewRecord() {
    ...
    $this->setFieldInitialValues($this->record, "DC0010");
    print "{success:true, data:".json_encode($this->record)."}";
...
private function _initFields() {
  ...
  ,"IS_PERIOD" => array(parent::FLDPROP_DATA_TYPE => "BOOLEAN")
  ,"IS_VOLUME" => array(parent::FLDPROP_DATA_TYPE => "BOOLEAN")
  ,"IS_WEIGHT" => array(parent::FLDPROP_DATA_TYPE => "BOOLEAN")
