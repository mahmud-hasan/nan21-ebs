 ...
 * Generated content.
 * DC0075 Controller: Price levels
 */

require_once(PATH_N21LIB."/Http/Controller.php");

class DC0075 extends Controller {
...
public function doQuery() {
    ...
    $sql = "select 
                ID
                ,NAME
            from MM_PRICE_LEVEL  $where $orderByClause ";
...
public function fetchRecord() {
    ...
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0075.fetchRecord().");}
...
public function doInsert() {
    ...
    $sql = "insert into MM_PRICE_LEVEL(
                 ID
                ,NAME
            ) values ( 
                 :ID
    ...
    $_seq = $this->getDbConn()->execute("select SEQ_PRICELVL_ID.nextval seq_val from dual")->fetchRow();
...
public function doDelete() {
    $this->record["ID"] = $this->getRequestParam("ID");
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0075.doDelete().");}
    $sql = "delete from MM_PRICE_LEVEL where 
           ID= :ID
    ";
    $stmt = $this->getDbConn()->prepare($sql);
    $this->getDbConn()->Execute($stmt, $this->record);
...
public function initNewRecord() {
    ...
    $this->setFieldInitialValues($this->record, "DC0075");
    print "{success:true, data:".json_encode($this->record)."}";
