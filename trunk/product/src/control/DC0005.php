 ...
 * Generated content.
 * DC0005 Controller: Banks
 */

require_once(PATH_CTRL_FRMWK."/Controller.php");

class DC0005 extends Controller {
...
private function preQuery(&$params, &$where) {
    ...
    if (!empty($_REQUEST["QRY_SWIFTCODE"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "SWIFTCODE like :SWIFTCODE";
      $params["SWIFTCODE"] = $_REQUEST["QRY_SWIFTCODE"];
    }
}
...
public function fetchRecord() {
  try {
    ...
    if (empty($RECORD["ID"])) { throw new Exception("Missing value for primary key field ID in DC0005.fetchRecord().");}
...
public function doInsert() {
  ...
  try {
    ...
    $RECORD["ID"] = $this->getRequestParam("ID");
    $RECORD["LOCATION_ID"] = $this->getRequestParam("LOCATION_ID");
...
public function doDelete() {
  ...
  try {
    $RECORD["ID"] = $this->getRequestParam("ID");
    if (empty($RECORD["ID"])) { throw new Exception("Missing value for primary key field ID in DC0005.doDelete().");}
...
public function initNewRecord() {
  try {
    ...
    $this->setFieldInitialValues($RECORD, "DC0005");
    print "{success:true, data:".json_encode($RECORD)."}";
  }catch(Exception  $e) {
    System::sendActionErrorJson( $e->getMessage());
  }
...
private function readRequest(&$RECORD) {
     ...
     if (isset($_REQUEST["LOCATION_ID"] )) { $RECORD["LOCATION_ID"] = $this->getRequestParam("LOCATION_ID"); }
     if (isset($_REQUEST["MODIFIEDBY"] )) { $RECORD["MODIFIEDBY"] = $this->getRequestParam("MODIFIEDBY"); }
     if (isset($_REQUEST["MODIFIEDON"] )) { $RECORD["MODIFIEDON"] = $this->getRequestParam("MODIFIEDON"); }
     if (isset($_REQUEST["NAME"] )) { $RECORD["NAME"] = $this->getRequestParam("NAME"); }
     if (isset($_REQUEST["SWIFTCODE"] )) { $RECORD["SWIFTCODE"] = $this->getRequestParam("SWIFTCODE"); }
