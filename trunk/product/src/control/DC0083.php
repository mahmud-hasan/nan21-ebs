...
public function fetchRecord() {
  try {
    ...
    if (empty($RECORD["ID"])) { throw new Exception("Missing value for primary key field ID in DC0083.fetchRecord().");}
...
public function doDelete() {
  ...
  try {
    $RECORD["ID"] = $this->getRequestParam("ID");
    if (empty($RECORD["ID"])) { throw new Exception("Missing value for primary key field ID in DC0083.doDelete().");}
...
public function initNewRecord() {
  try {
    ...
    $this->setFieldInitialValues($RECORD, "DC0083");
    print "{success:true, data:".json_encode($RECORD)."}";
  }catch(Exception  $e) {
    System::sendActionErrorJson( $e->getMessage());
  }
