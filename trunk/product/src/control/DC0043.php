 ...
 * Generated content.
 * DC0043 Controller: Menu item translations
 */

require_once(PATH_CTRL_FRMWK."/Controller.php");

class DC0043 extends Controller {
...
public function fetchRecord() {
  try {
    ...
    if (empty($RECORD["ID"])) { throw new Exception("Missing value for primary key field ID in DC0043.fetchRecord().");}
...
public function doInsert() {
  ...
  try {
    ...
    $RECORD["ID"] = $this->getRequestParam("ID");
    $RECORD["LANG"] = $this->getRequestParam("LANG");
...
public function doDelete() {
  ...
  try {
    $RECORD["ID"] = $this->getRequestParam("ID");
    if (empty($RECORD["ID"])) { throw new Exception("Missing value for primary key field ID in DC0043.doDelete().");}
    $sql = "delete from MENUITEM_TRL where 
           ID= :ID
    ";
    $stmt = $this->db->prepare($sql);
    $this->db->Execute($stmt, $RECORD);
...
public function initNewRecord() {
  try {
    ...
    $this->setFieldInitialValues($RECORD, "DC0043");
    print "{success:true, data:".json_encode($RECORD)."}";
  }catch(Exception  $e) {
    System::sendActionErrorJson( $e->getMessage());
  }
...
private function readRequest(&$RECORD) {
     ...
     if (isset($_REQUEST["LANG"] )) { $RECORD["LANG"] = $this->getRequestParam("LANG"); }
