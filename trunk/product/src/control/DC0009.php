...
public function fetchRecord() {
  try {
    ...
    if (empty($RECORD["ID"])) { throw new Exception("Missing value for primary key field ID in DC0009.fetchRecord().");}
...
public function doInsert() {
  ...
  try {
    ...
    $RECORD["AREA_CODE"] = $this->getRequestParam("AREA_CODE");
...
public function doDelete() {
  ...
  try {
    $RECORD["ID"] = $this->getRequestParam("ID");
    if (empty($RECORD["ID"])) { throw new Exception("Missing value for primary key field ID in DC0009.doDelete().");}
...
public function initNewRecord() {
  try {
    ...
    $this->setFieldInitialValues($RECORD, "DC0009");
    print "{success:true, data:".json_encode($RECORD)."}";
  }catch(Exception  $e) {
    System::sendActionErrorJson( $e->getMessage());
  }
...
private function readRequest(&$RECORD) {
    $RECORD["ACTIVE"] = $this->getRequestParamBoolean("ACTIVE");
     if (isset($_REQUEST["AREA_CODE"] )) { $RECORD["AREA_CODE"] = $this->getRequestParam("AREA_CODE"); }
     ...
     if (isset($_REQUEST["POSTAL_CODE"] )) { $RECORD["POSTAL_CODE"] = $this->getRequestParam("POSTAL_CODE"); }
