...
private function preQuery(&$params, &$where) {
    ...
    if (!empty($_REQUEST["QRY_CONTENT"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "CONTENT like :CONTENT";
      $params["CONTENT"] = $_REQUEST["QRY_CONTENT"];
    }
    ...
    if (!empty($_REQUEST["QRY_DECLARED_VALUE"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "DECLARED_VALUE like :DECLARED_VALUE";
      $params["DECLARED_VALUE"] = $_REQUEST["QRY_DECLARED_VALUE"];
    }
    ...
    if (!empty($_REQUEST["QRY_DELIVERED_TO_IDENT"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "DELIVERED_TO_IDENT like :DELIVERED_TO_IDENT";
      $params["DELIVERED_TO_IDENT"] = $_REQUEST["QRY_DELIVERED_TO_IDENT"];
    }
    if (!empty($_REQUEST["QRY_DELIVERED_TO_NAME"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "DELIVERED_TO_NAME like :DELIVERED_TO_NAME";
      $params["DELIVERED_TO_NAME"] = $_REQUEST["QRY_DELIVERED_TO_NAME"];
    }
    ...
    if (!empty($_REQUEST["QRY_DELIVERY_MODE"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "DELIVERY_MODE like :DELIVERY_MODE";
      $params["DELIVERY_MODE"] = $_REQUEST["QRY_DELIVERY_MODE"];
    }
    if (!empty($_REQUEST["QRY_DEST_ADRESS_NOTE"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "DEST_ADRESS_NOTE like :DEST_ADRESS_NOTE";
      $params["DEST_ADRESS_NOTE"] = $_REQUEST["QRY_DEST_ADRESS_NOTE"];
    }
    if (!empty($_REQUEST["QRY_DEST_BPADRESS"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "DEST_BPADRESS like :DEST_BPADRESS";
      $params["DEST_BPADRESS"] = $_REQUEST["QRY_DEST_BPADRESS"];
    }
    ...
    if (!empty($_REQUEST["QRY_DEST_ZIP"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "DEST_ZIP like :DEST_ZIP";
      $params["DEST_ZIP"] = $_REQUEST["QRY_DEST_ZIP"];
    }
    if (!empty($_REQUEST["QRY_EXP_ADRESS_NOTE"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "EXP_ADRESS_NOTE like :EXP_ADRESS_NOTE";
      $params["EXP_ADRESS_NOTE"] = $_REQUEST["QRY_EXP_ADRESS_NOTE"];
    }
    ...
    if (!empty($_REQUEST["QRY_EXP_ZIP"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "EXP_ZIP like :EXP_ZIP";
      $params["EXP_ZIP"] = $_REQUEST["QRY_EXP_ZIP"];
    }
    ...
    if (!empty($_REQUEST["QRY_INSURED_VALUE"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "INSURED_VALUE like :INSURED_VALUE";
      $params["INSURED_VALUE"] = $_REQUEST["QRY_INSURED_VALUE"];
    }
    ...
    if (!empty($_REQUEST["QRY_PACKAGE_COUNT"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "PACKAGE_COUNT like :PACKAGE_COUNT";
      $params["PACKAGE_COUNT"] = $_REQUEST["QRY_PACKAGE_COUNT"];
    }
    if (!empty($_REQUEST["QRY_PICKEDUP"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "PICKEDUP like :PICKEDUP";
      $params["PICKEDUP"] = $_REQUEST["QRY_PICKEDUP"];
    }
    ...
    if (!empty($_REQUEST["QRY_PICKUP_MODE"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "PICKUP_MODE like :PICKUP_MODE";
      $params["PICKUP_MODE"] = $_REQUEST["QRY_PICKUP_MODE"];
    }
    if (!empty($_REQUEST["QRY_REF_PARCEL_ID"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "REF_PARCEL_ID like :REF_PARCEL_ID";
      $params["REF_PARCEL_ID"] = $_REQUEST["QRY_REF_PARCEL_ID"];
    }
    if (!empty($_REQUEST["QRY_REF_PARCEL_REFERENCE_TYPE"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "REF_PARCEL_REFERENCE_TYPE like :REF_PARCEL_REFERENCE_TYPE";
      $params["REF_PARCEL_REFERENCE_TYPE"] = $_REQUEST["QRY_REF_PARCEL_REFERENCE_TYPE"];
    }
    ...
    if (!empty($_REQUEST["QRY_REJECTED_BY_IDENT"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "REJECTED_BY_IDENT like :REJECTED_BY_IDENT";
      $params["REJECTED_BY_IDENT"] = $_REQUEST["QRY_REJECTED_BY_IDENT"];
    }
    if (!empty($_REQUEST["QRY_REJECTED_BY_NAME"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "REJECTED_BY_NAME like :REJECTED_BY_NAME";
      $params["REJECTED_BY_NAME"] = $_REQUEST["QRY_REJECTED_BY_NAME"];
    }
    if (!empty($_REQUEST["QRY_REJECT_DATE"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "REJECT_DATE like :REJECT_DATE";
      $params["REJECT_DATE"] = $_REQUEST["QRY_REJECT_DATE"];
    }
    if (!empty($_REQUEST["QRY_REJECT_REASON"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "REJECT_REASON like :REJECT_REASON";
      $params["REJECT_REASON"] = $_REQUEST["QRY_REJECT_REASON"];
    }
}
