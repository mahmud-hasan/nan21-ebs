 ...
 * Generated content.
 * DC0104 Controller: Product client list
 */

require_once(PATH_CTRL_FRMWK."/Controller.php");

class DC0104 extends Controller {
...
public function doQuery() {
  try {
    ...
    $sql = "select 
                ...
                ,t.PRODCLASS_CODE
                ,t.PRODUCT_ID
            from MM_PRODUCT_CLIENT t $where $orderByClause ";
...
public function fetchRecord() {
  try {
    ...
    if (empty($RECORD["ID"])) { throw new Exception("Missing value for primary key field ID in DC0104.fetchRecord().");}
...
public function doInsert() {
  ...
  try {
    ...
    $RECORD["PRODCLASS_CODE"] = $this->getRequestParam("PRODCLASS_CODE");
    $RECORD["PRODUCT_ID"] = $this->getRequestParam("PRODUCT_ID");
    $sql = "insert into MM_PRODUCT_CLIENT(
                 CLIENT_ID
                ,FOR_SALE
                ,ID
                ,PRODCLASS_CODE
    ...
    $_seq = $this->db->execute("select SEQ_PRODCLIENT_ID.nextval seq_val from dual")->fetchRow();
...
public function doUpdate() {
  try {
    ...
    $RECORD["PRODCLASS_CODE"] = $this->getRequestParam("PRODCLASS_CODE");
    $RECORD["PRODUCT_ID"] = $this->getRequestParam("PRODUCT_ID");
    $sql = "update MM_PRODUCT_CLIENT set 
                 CLIENT_ID=:CLIENT_ID
                ,CREATEDBY=:CREATEDBY
                ,CREATEDON=:CREATEDON
                ,FOR_SALE=:FOR_SALE
...
public function doDelete() {
  ...
  try {
    $RECORD["ID"] = $this->getRequestParam("ID");
    if (empty($RECORD["ID"])) { throw new Exception("Missing value for primary key field ID in DC0104.doDelete().");}
    $sql = "delete from MM_PRODUCT_CLIENT where 
           ID= :ID
    ";
    $stmt = $this->db->prepare($sql);
    $this->db->Execute($stmt, $RECORD);
...
public function initNewRecord() {
  try {
    ...
    $this->setFieldInitialValues($RECORD, "DC0104");
    print "{success:true, data:".json_encode($RECORD)."}";
  }catch(Exception  $e) {
    System::sendActionErrorJson( $e->getMessage());
  }
...
private  $fieldDef = array(
  ...
  ,"PRODCLASS_CODE" => array("DATA_TYPE" => "STRING")
...
private function readRequest(&$RECORD) {
    ...
    $RECORD["FOR_SALE"] = $this->getRequestParamBoolean("FOR_SALE");
     ...
     if (isset($_REQUEST["PRODCLASS_CODE"] )) { $RECORD["PRODCLASS_CODE"] = $this->getRequestParam("PRODCLASS_CODE"); }
