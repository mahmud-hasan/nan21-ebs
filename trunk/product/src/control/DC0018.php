...
public function fetchRecord() {
  try {
    ...
    if (empty($RECORD["ID"])) { throw new Exception("Missing value for primary key field ID in DC0018.fetchRecord().");}
...
public function doInsert() {
  ...
  try {
    ...
    $RECORD["BANKAG_CODE"] = $this->getRequestParam("BANKAG_CODE");
    $RECORD["BANK_CODE"] = $this->getRequestParam("BANK_CODE");
    $RECORD["BPARTNER_ID"] = $this->getRequestParam("BPARTNER_ID");
    $RECORD["CREATEDBY"] = $this->getRequestParam("CREATEDBY");
    $RECORD["CREATEDON"] = $this->getRequestParam("CREATEDON");
    $RECORD["CURRENCY"] = $this->getRequestParam("CURRENCY");
    $RECORD["IBAN"] = $this->getRequestParam("IBAN");
    $RECORD["ID"] = $this->getRequestParam("ID");
...
public function doDelete() {
  ...
  try {
    $RECORD["ID"] = $this->getRequestParam("ID");
    if (empty($RECORD["ID"])) { throw new Exception("Missing value for primary key field ID in DC0018.doDelete().");}
...
public function initNewRecord() {
  try {
    ...
    $this->setFieldInitialValues($RECORD, "DC0018");
    print "{success:true, data:".json_encode($RECORD)."}";
  }catch(Exception  $e) {
    System::sendActionErrorJson( $e->getMessage());
  }
...
private function readRequest(&$RECORD) {
     if (isset($_REQUEST["BANKAG_CODE"] )) { $RECORD["BANKAG_CODE"] = $this->getRequestParam("BANKAG_CODE"); }
     if (isset($_REQUEST["BANK_CODE"] )) { $RECORD["BANK_CODE"] = $this->getRequestParam("BANK_CODE"); }
     ...
     if (isset($_REQUEST["IBAN"] )) { $RECORD["IBAN"] = $this->getRequestParam("IBAN"); }
