 ...
 * Generated content.
 * DC0045 Controller: Projects
 */

require_once(PATH_CTRL_FRMWK."/Controller.php");

class DC0045 extends Controller {
...
private function preQuery(&$params, &$where) {
    ...
    if (!empty($_REQUEST["QRY_TYPE_CODE"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "TYPE_CODE like :TYPE_CODE";
      $params["TYPE_CODE"] = $_REQUEST["QRY_TYPE_CODE"];
    }
}
...
public function fetchRecord() {
  try {
    ...
    if (empty($RECORD["ID"])) { throw new Exception("Missing value for primary key field ID in DC0045.fetchRecord().");}
...
public function doInsert() {
  ...
  try {
    ...
    $_seq = $this->db->execute("select seq_project_id.nextval seq_val from dual")->fetchRow();
...
public function doDelete() {
  ...
  try {
    $RECORD["ID"] = $this->getRequestParam("ID");
    if (empty($RECORD["ID"])) { throw new Exception("Missing value for primary key field ID in DC0045.doDelete().");}
...
public function initNewRecord() {
  try {
    ...
    $this->setFieldInitialValues($RECORD, "DC0045");
    print "{success:true, data:".json_encode($RECORD)."}";
  }catch(Exception  $e) {
    System::sendActionErrorJson( $e->getMessage());
  }
...
private function readRequest(&$RECORD) {
     ...
     if (isset($_REQUEST["TYPE_CODE"] )) { $RECORD["TYPE_CODE"] = $this->getRequestParam("TYPE_CODE"); }
