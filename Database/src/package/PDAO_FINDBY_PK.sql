 ...
 */
  ...
  procedure imp_file_t(p_id in imp_file.id%type, p_record in out imp_file%rowtype, p_raise in char default 'N');
  ...
  procedure org_attr_t(p_id in org_attr.id%type, p_record in out org_attr%rowtype, p_raise in char default 'N');
  procedure org_attr_val_t(p_id in org_attr_val.id%type, p_record in out org_attr_val%rowtype, p_raise in char default 'N');
  ...
  procedure tr_parcel_event_t(p_id in tr_parcel_event.id%type, p_record in out tr_parcel_event%rowtype, p_raise in char default 'N');
  procedure tr_parcel_event_type_t(p_id in tr_parcel_event_type.id%type, p_record in out tr_parcel_event_type%rowtype, p_raise in char default 'N');
  procedure tr_parcel_event_type_grp_t(p_code in tr_parcel_event_type_grp.code%type, p_record in out tr_parcel_event_type_grp%rowtype, p_raise in char default 'N');
  procedure tr_parcel_reject_reason_t(p_id in tr_parcel_reject_reason.id%type, p_record in out tr_parcel_reject_reason%rowtype, p_raise in char default 'N');
...
procedure imp_file_t(p_id in imp_file.id%type, p_record in out imp_file%rowtype, p_raise in char default 'N') is
  cursor c is 
    select t.* from imp_file t where t.id = p_id; 
begin
  open c;
  fetch c into p_record;
  if c%notfound and p_raise = 'Y' then 
    raise_error('DaoRecordByPkNotFound' , 'IMP_FILE');
  end if;
  close c;
end;
...
procedure org_attr_t(p_id in org_attr.id%type, p_record in out org_attr%rowtype, p_raise in char default 'N') is
  cursor c is 
    select t.* from org_attr t where t.id = p_id; 
begin
  open c;
  fetch c into p_record;
  if c%notfound and p_raise = 'Y' then 
    raise_error('DaoRecordByPkNotFound' , 'ORG_ATTR');
  end if;
  close c;
end;


procedure org_attr_val_t(p_id in org_attr_val.id%type, p_record in out org_attr_val%rowtype, p_raise in char default 'N') is
  cursor c is 
    select t.* from org_attr_val t where t.id = p_id; 
begin
  open c;
  fetch c into p_record;
  if c%notfound and p_raise = 'Y' then 
    raise_error('DaoRecordByPkNotFound' , 'ORG_ATTR_VAL');
  end if;
  close c;
end;
...
procedure tr_parcel_event_t(p_id in tr_parcel_event.id%type, p_record in out tr_parcel_event%rowtype, p_raise in char default 'N') is
  cursor c is 
    select t.* from tr_parcel_event t where t.id = p_id; 
begin
  open c;
  fetch c into p_record;
  if c%notfound and p_raise = 'Y' then 
    raise_error('DaoRecordByPkNotFound' , 'TR_PARCEL_EVENT');
  end if;
  close c;
end;


procedure tr_parcel_event_type_t(p_id in tr_parcel_event_type.id%type, p_record in out tr_parcel_event_type%rowtype, p_raise in char default 'N') is
  cursor c is 
    select t.* from tr_parcel_event_type t where t.id = p_id; 
begin
  open c;
  fetch c into p_record;
  if c%notfound and p_raise = 'Y' then 
    raise_error('DaoRecordByPkNotFound' , 'TR_PARCEL_EVENT_TYPE');
  end if;
  close c;
end;


procedure tr_parcel_event_type_grp_t(p_code in tr_parcel_event_type_grp.code%type, p_record in out tr_parcel_event_type_grp%rowtype, p_raise in char default 'N') is
  cursor c is 
    select t.* from tr_parcel_event_type_grp t where t.code = p_code; 
begin
  open c;
  fetch c into p_record;
  if c%notfound and p_raise = 'Y' then 
    raise_error('DaoRecordByPkNotFound' , 'TR_PARCEL_EVENT_TYPE_GRP');
  end if;
  close c;
end;


procedure tr_parcel_reject_reason_t(p_id in tr_parcel_reject_reason.id%type, p_record in out tr_parcel_reject_reason%rowtype, p_raise in char default 'N') is
  cursor c is 
    select t.* from tr_parcel_reject_reason t where t.id = p_id; 
begin
  open c;
  fetch c into p_record;
  if c%notfound and p_raise = 'Y' then 
    raise_error('DaoRecordByPkNotFound' , 'TR_PARCEL_REJECT_REASON');
  end if;
  close c;
end;
