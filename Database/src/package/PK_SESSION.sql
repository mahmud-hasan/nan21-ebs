...
create or replace package body pk_session is
  ...
  /**
   * Returns the current user default client, i.e. the client on which the current user is authorized to work with.
   * For the moment returns the default client per application. 
   * Todo: update this when infrastructure ready for user preferences      
   */
  function get_default_client_id return number is
    cursor c is 
      select id from client where DEFAULT_CLIENT = 'Y';
    r client.id%type;  
  begin
    open c;
    fetch c into r;
