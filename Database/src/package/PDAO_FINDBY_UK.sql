 ...
 */
  ...
  procedure document_serial_no_t(p_client_id in document_serial_no.client_id%type, p_docser_id in document_serial_no.docser_id%type, p_docserrng_id in document_serial_no.docserrng_id%type, p_value in document_serial_no.value%type, p_record in out document_serial_no%rowtype, p_raise in char default 'N');
  ...
  procedure org_attr_t(p_name in org_attr.name%type, p_record in out org_attr%rowtype, p_raise in char default 'N');
  procedure org_attr_val_t(p_org_id in org_attr_val.org_id%type, p_orgattr_id in org_attr_val.orgattr_id%type, p_record in out org_attr_val%rowtype, p_raise in char default 'N');
  ...
  procedure tr_parcel_event_type_t(p_code in tr_parcel_event_type.code%type, p_record in out tr_parcel_event_type%rowtype, p_raise in char default 'N');
  procedure tr_parcel_reject_reason_t(p_code in tr_parcel_reject_reason.code%type, p_record in out tr_parcel_reject_reason%rowtype, p_raise in char default 'N');
...
procedure document_serial_no_t(p_client_id in document_serial_no.client_id%type, p_docser_id in document_serial_no.docser_id%type, p_docserrng_id in document_serial_no.docserrng_id%type, p_value in document_serial_no.value%type, p_record in out document_serial_no%rowtype, p_raise in char default 'N') is
  cursor c is 
    select t.* from document_serial_no t where t.client_id = p_client_id and t.docser_id = p_docser_id and t.docserrng_id = p_docserrng_id and t.value = p_value; 
begin
  open c;
  fetch c into p_record;
  if c%notfound and p_raise = 'Y' then 
    raise_error('DaoRecordByUkNotFound' , 'DOCUMENT_SERIAL_NO');
  end if;
  close c;
end;
...
procedure org_attr_t(p_name in org_attr.name%type, p_record in out org_attr%rowtype, p_raise in char default 'N') is
  cursor c is 
    select t.* from org_attr t where t.name = p_name; 
begin
  open c;
  fetch c into p_record;
  if c%notfound and p_raise = 'Y' then 
    raise_error('DaoRecordByUkNotFound' , 'ORG_ATTR');
  end if;
  close c;
end;


procedure org_attr_val_t(p_org_id in org_attr_val.org_id%type, p_orgattr_id in org_attr_val.orgattr_id%type, p_record in out org_attr_val%rowtype, p_raise in char default 'N') is
  cursor c is 
    select t.* from org_attr_val t where t.org_id = p_org_id and t.orgattr_id = p_orgattr_id; 
begin
  open c;
  fetch c into p_record;
  if c%notfound and p_raise = 'Y' then 
    raise_error('DaoRecordByUkNotFound' , 'ORG_ATTR_VAL');
  end if;
  close c;
end;
...
procedure tr_parcel_event_type_t(p_code in tr_parcel_event_type.code%type, p_record in out tr_parcel_event_type%rowtype, p_raise in char default 'N') is
  cursor c is 
    select t.* from tr_parcel_event_type t where t.code = p_code; 
begin
  open c;
  fetch c into p_record;
  if c%notfound and p_raise = 'Y' then 
    raise_error('DaoRecordByUkNotFound' , 'TR_PARCEL_EVENT_TYPE');
  end if;
  close c;
end;


procedure tr_parcel_reject_reason_t(p_code in tr_parcel_reject_reason.code%type, p_record in out tr_parcel_reject_reason%rowtype, p_raise in char default 'N') is
  cursor c is 
    select t.* from tr_parcel_reject_reason t where t.code = p_code; 
begin
  open c;
  fetch c into p_record;
  if c%notfound and p_raise = 'Y' then 
    raise_error('DaoRecordByUkNotFound' , 'TR_PARCEL_REJECT_REASON');
  end if;
  close c;
end;
