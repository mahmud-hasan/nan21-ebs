 ...
 * Generated content.
 * DC0099 Controller: Means of transp types
 */

require_once(PATH_N21LIB."/Http/Controller.php");

class DC0099 extends Controller {
...
public function doQuery() {
    ...
    $sql = "select 
                ...
                ,t.MODIFIEDON
                ,t.NAME
            from TR_VEHICLE_TYPE t $where $orderByClause ";
...
public function fetchRecord() {
    ...
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0099.fetchRecord().");}
...
public function doInsert() {
    ...
    $sql = "insert into TR_VEHICLE_TYPE(
                 ACTIVE
                ,CODE
                ,ID
                ,NAME
    ...
    $_seq = $this->getDbConn()->execute("select SEQ_VEHICLETYP_ID.nextval seq_val from dual")->fetchRow();
...
public function doUpdate() {
    ...
    $this->record["_p_record_status"] = $this->getRequestParam("_p_record_status");
    $this->record["_p_store_recId"] = $this->getRequestParam("_p_store_recId");
    $sql = "update TR_VEHICLE_TYPE set 
                 ACTIVE=:ACTIVE
                ,CREATEDBY=:CREATEDBY
                ,CREATEDON=:CREATEDON
                ,ID=:ID
...
public function doDelete() {
    $this->record["ID"] = $this->getRequestParam("ID");
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0099.doDelete().");}
    $sql = "delete from TR_VEHICLE_TYPE where 
           ID= :ID
    ";
    $stmt = $this->getDbConn()->prepare($sql);
    $this->getDbConn()->Execute($stmt, $this->record);
...
public function initNewRecord() {
    ...
    $this->setFieldInitialValues($this->record, "DC0099");
    print "{success:true, data:".json_encode($this->record)."}";
