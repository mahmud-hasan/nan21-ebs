 ...
 * Generated content.
 * DC0006 Controller: Bank agencies
 */

require_once(PATH_N21LIB."/Http/Controller.php");

class DC0006 extends Controller {
...
private function preQuery(&$params, &$where) {
    if (!empty($_REQUEST["QRY_BANK_CODE"])) {
      ...
      $params["BANK_CODE"] = strtoupper($params["BANK_CODE"]);
    }
...
public function fetchRecord() {
    ...
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0006.fetchRecord().");}
...
public function doInsert() {
    ...
    $_seq = $this->getDbConn()->execute("select seq_bankag_id.nextval seq_val from dual")->fetchRow();
...
public function doDelete() {
    $this->record["ID"] = $this->getRequestParam("ID");
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0006.doDelete().");}
    $sql = "delete from BANK_AGENCY where 
           ID= :ID
    ";
    $stmt = $this->getDbConn()->prepare($sql);
    $this->getDbConn()->Execute($stmt, $this->record);
...
public function initNewRecord() {
    ...
    $this->setFieldInitialValues($this->record, "DC0006");
    print "{success:true, data:".json_encode($this->record)."}";
...
private function _initFields() {
  ...
  ,"BANK_CODE" => array(parent::FLDPROP_DATA_TYPE => "STRING",parent::FLDPROP_CASE_RESTRICTION => "Upper")
