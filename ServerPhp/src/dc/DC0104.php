...
public function fetchRecord() {
    ...
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0104.fetchRecord().");}
...
public function doInsert() {
    ...
    $_seq = $this->getDbConn()->execute("select SEQ_PRODCLIENT_ID.nextval seq_val from dual")->fetchRow();
...
public function doDelete() {
    $this->record["ID"] = $this->getRequestParam("ID");
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0104.doDelete().");}
...
public function initNewRecord() {
    ...
    $this->setFieldInitialValues($this->record, "DC0104");
    print "{success:true, data:".json_encode($this->record)."}";
...
private function _initFields() {
  ...
  ,"FOR_SALE" => array(parent::FLDPROP_DATA_TYPE => "BOOLEAN")
  ...
  ,"PRODCLASS_CODE" => array(parent::FLDPROP_DATA_TYPE => "STRING")
