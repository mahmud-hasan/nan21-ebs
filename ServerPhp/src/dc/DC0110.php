 ...
 * Generated content.
 * DC0110 Controller: Parcel return reasons
 */

require_once(PATH_N21LIB."/Http/Controller.php");

class DC0110 extends Controller {
...
public function doQuery() {
    ...
    $sql = "select 
                ...
                ,t.MODIFIEDON
                ,t.NAME
            from TR_PARCEL_REJECT_REASON t $where $orderByClause ";
...
public function fetchRecord() {
    ...
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0110.fetchRecord().");}
...
public function doInsert() {
    ...
    $sql = "insert into TR_PARCEL_REJECT_REASON(
                 ACTIVE
                ,CODE
                ,ID
                ,NAME
    ...
    $_seq = $this->getDbConn()->execute("select SEQ_PARCELREJRES_ID.nextval seq_val from dual")->fetchRow();
...
public function doUpdate() {
    ...
    $this->record["_p_record_status"] = $this->getRequestParam("_p_record_status");
    $this->record["_p_store_recId"] = $this->getRequestParam("_p_store_recId");
    $sql = "update TR_PARCEL_REJECT_REASON set 
                 ACTIVE=:ACTIVE
                ,CREATEDBY=:CREATEDBY
                ,CREATEDON=:CREATEDON
                ,ID=:ID
...
public function doDelete() {
    $this->record["ID"] = $this->getRequestParam("ID");
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0110.doDelete().");}
    $sql = "delete from TR_PARCEL_REJECT_REASON where 
           ID= :ID
    ";
    $stmt = $this->getDbConn()->prepare($sql);
    $this->getDbConn()->Execute($stmt, $this->record);
...
public function initNewRecord() {
    ...
    $this->setFieldInitialValues($this->record, "DC0110");
    print "{success:true, data:".json_encode($this->record)."}";
...
private function findByPk(&$pkCols, &$record) {
    $sql = "select 
                ...
                ,t.MODIFIEDON
                ,t.NAME
            from TR_PARCEL_REJECT_REASON t
         where 
           t.ID= :ID
            ";
