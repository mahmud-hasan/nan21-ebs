 ...
 * Generated content.
 * DC0025 Controller: UI lovs
 */

require_once(PATH_N21LIB."/Http/Controller.php");

class DC0025 extends Controller {
...
public function fetchRecord() {
    ...
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0025.fetchRecord().");}
...
public function doInsert() {
    ...
    $_seq = $this->getDbConn()->execute("select SEQ_UILOV_ID.nextval seq_val from dual")->fetchRow();
...
public function doUpdate() {
    $this->populateRecordFromRequest();
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0025.doUpdate().");}
    $sql = "update UI_LOV set 
                 ID=:ID
                ,CODE=:CODE
                ,NAME=:NAME
...
public function doDelete() {
    $this->record["ID"] = $this->getRequestParam("ID");
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0025.doDelete().");}
    $sql = "delete from UI_LOV where 
           ID= :ID
    ";
    $stmt = $this->getDbConn()->prepare($sql);
    $this->getDbConn()->Execute($stmt, $this->record);
...
public function initNewRecord() {
    ...
    $this->setFieldInitialValues($this->record, "DC0025");
    print "{success:true, data:".json_encode($this->record)."}";
