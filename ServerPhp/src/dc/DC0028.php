...
public function fetchRecord() {
    ...
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0028.fetchRecord().");}
...
public function doInsert() {
    ...
    $_seq = $this->getDbConn()->execute("select SEQ_PAYDOCTYPE_ID.nextval seq_val from dual")->fetchRow();
...
public function doDelete() {
    $this->record["ID"] = $this->getRequestParam("ID");
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0028.doDelete().");}
...
public function initNewRecord() {
    ...
    $this->setFieldInitialValues($this->record, "DC0028");
    print "{success:true, data:".json_encode($this->record)."}";
...
private function _initFields() {
  ...
  ,"COMPENSATION" => array(parent::FLDPROP_DATA_TYPE => "BOOLEAN")
  ...
  ,"PAYABLE" => array(parent::FLDPROP_DATA_TYPE => "BOOLEAN")
  ,"PRINT_REPORT" => array(parent::FLDPROP_DATA_TYPE => "STRING")
  ,"RECEIVABLE" => array(parent::FLDPROP_DATA_TYPE => "BOOLEAN")
);
}
