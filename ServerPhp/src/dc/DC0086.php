 ...
 * Generated content.
 * DC0086 Controller: Org. inventory type
 */

require_once(PATH_N21LIB."/Http/Controller.php");

class DC0086 extends Controller {
...
public function doQuery() {
    ...
    $sql = "select 
                ...
                ,t.MODIFIEDBY
                ,t.MODIFIEDON
            from MM_ORG_INV_TYPE t $where $orderByClause ";
...
public function fetchRecord() {
    ...
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0086.fetchRecord().");}
...
public function doInsert() {
    ...
    $sql = "insert into MM_ORG_INV_TYPE(
                 CODE
                ,DESCRIPTION
                ,ID
            ) values ( 
    ...
    $_seq = $this->getDbConn()->execute("select SEQ_ORGINVTYP_ID.nextval seq_val from dual")->fetchRow();
...
public function doUpdate() {
    ...
    $this->record["_p_record_status"] = $this->getRequestParam("_p_record_status");
    $this->record["_p_store_recId"] = $this->getRequestParam("_p_store_recId");
    $sql = "update MM_ORG_INV_TYPE set 
                 CODE=:CODE
                ,CREATEDBY=:CREATEDBY
                ,CREATEDON=:CREATEDON
...
public function doDelete() {
    $this->record["ID"] = $this->getRequestParam("ID");
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0086.doDelete().");}
    $sql = "delete from MM_ORG_INV_TYPE where 
           ID= :ID
    ";
    $stmt = $this->getDbConn()->prepare($sql);
    $this->getDbConn()->Execute($stmt, $this->record);
...
public function initNewRecord() {
    ...
    $this->setFieldInitialValues($this->record, "DC0086");
    print "{success:true, data:".json_encode($this->record)."}";
