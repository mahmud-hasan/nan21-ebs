 ...
 * Generated content.
 * DC0062 Controller: Asset groups
 */

require_once(PATH_N21LIB."/Http/Controller.php");

class DC0062 extends Controller {
...
public function fetchRecord() {
    ...
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0062.fetchRecord().");}
...
public function doInsert() {
    ...
    $_seq = $this->getDbConn()->execute("select seq_assetgrp_id.nextval seq_val from dual")->fetchRow();
...
public function doDelete() {
    $this->record["ID"] = $this->getRequestParam("ID");
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0062.doDelete().");}
    $sql = "delete from ASSET_GROUP where 
           ID= :ID
    ";
    $stmt = $this->getDbConn()->prepare($sql);
    $this->getDbConn()->Execute($stmt, $this->record);
...
public function initNewRecord() {
    ...
    $this->setFieldInitialValues($this->record, "DC0062");
    print "{success:true, data:".json_encode($this->record)."}";
