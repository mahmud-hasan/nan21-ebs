...
public function fetchRecord() {
    ...
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0005.fetchRecord().");}
...
public function doInsert() {
    ...
    $_seq = $this->getDbConn()->execute("select seq_bank_id.nextval seq_val from dual")->fetchRow();
...
public function doDelete() {
    $this->record["ID"] = $this->getRequestParam("ID");
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0005.doDelete().");}
...
public function initNewRecord() {
    ...
    $this->setFieldInitialValues($this->record, "DC0005");
    print "{success:true, data:".json_encode($this->record)."}";
...
private function _initFields() {
  ...
  ,"LOCATION_ID" => array(parent::FLDPROP_DATA_TYPE => "NUMBER")
  ...
  ,"SWIFTCODE" => array(parent::FLDPROP_DATA_TYPE => "STRING")
);
}
