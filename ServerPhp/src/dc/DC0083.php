 ...
 * Generated content.
 * DC0083 Controller: Cost centers
 */

require_once(PATH_N21LIB."/Http/Controller.php");

class DC0083 extends Controller {
...
private function preQuery(&$params, &$where) {
    ...
    if (!empty($_REQUEST["QRY_CLIENT_ID"])) {
      $where .= (!empty($where))?" and ":"";
      $where .= "cc.CLIENT_ID like :CLIENT_ID";
...
public function fetchRecord() {
    ...
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0083.fetchRecord().");}
...
public function doInsert() {
    ...
    $_seq = $this->getDbConn()->execute("select SEQ_COSTCENTER_ID.nextval seq_val from dual")->fetchRow();
...
public function doDelete() {
    $this->record["ID"] = $this->getRequestParam("ID");
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0083.doDelete().");}
...
public function initNewRecord() {
    ...
    $this->setFieldInitialValues($this->record, "DC0083");
    print "{success:true, data:".json_encode($this->record)."}";
