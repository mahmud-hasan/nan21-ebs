 ...
 * Generated content.
 * DC0095 Controller: Transport type
 */

require_once(PATH_N21LIB."/Http/Controller.php");

class DC0095 extends Controller {
...
public function doQuery() {
    ...
    $sql = "select 
                ...
                ,t.MODIFIEDON
                ,t.NAME
            from TR_TRANSPORT_TYPE t $where $orderByClause ";
...
public function fetchRecord() {
    ...
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0095.fetchRecord().");}
...
public function doInsert() {
    ...
    $sql = "insert into TR_TRANSPORT_TYPE(
                 ACTIVE
                ,CODE
                ,ID
                ,NAME
    ...
    $_seq = $this->getDbConn()->execute("select SEQ_TRANSPTYPE_ID.nextval seq_val from dual")->fetchRow();
...
public function doUpdate() {
    ...
    $this->record["_p_record_status"] = $this->getRequestParam("_p_record_status");
    $this->record["_p_store_recId"] = $this->getRequestParam("_p_store_recId");
    $sql = "update TR_TRANSPORT_TYPE set 
                 ACTIVE=:ACTIVE
                ,CREATEDBY=:CREATEDBY
                ,CREATEDON=:CREATEDON
                ,ID=:ID
...
public function doDelete() {
    $this->record["ID"] = $this->getRequestParam("ID");
    if (empty($this->record["ID"])) { throw new Exception("Missing value for primary key field ID in DC0095.doDelete().");}
    $sql = "delete from TR_TRANSPORT_TYPE where 
           ID= :ID
    ";
    $stmt = $this->getDbConn()->prepare($sql);
    $this->getDbConn()->Execute($stmt, $this->record);
...
public function initNewRecord() {
    ...
    $this->setFieldInitialValues($this->record, "DC0095");
    print "{success:true, data:".json_encode($this->record)."}";
